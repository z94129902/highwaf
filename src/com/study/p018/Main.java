...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("====== Java 高级练习 第 18 题 ======");
        System.out.println("题目：ArrayList 按下标操作");
        System.out.println("说明：创建一个 ArrayList<Integer>，练习通过下标操作元素。");
        System.out.println();
        run();
    }
    ...
    private static void solve() throws Exception {
        ...
        int index = readInt("请输入要操作的下标：");
        if (index < 0 || index >= numberList.size()) {
            System.out.println("下标越界。");
            return;
        }
        int value = numberList.get(index);
        System.out.println("获取到的元素：" + value);

        int newValue = readInt("请输入新值：");
        numberList.set(index, newValue);

        System.out.println("修改后的集合：" + numberList);
